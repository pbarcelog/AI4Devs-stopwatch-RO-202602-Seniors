...
# Constraints:
1. base the implementation on the files present in the project, especially index.html and script.js,
...
3. Enable any necessary setup and/or configuration needed to run the tests locally
...
- another button to 'Reset', that takes the countdown back to 00:00:00 when pressed, hiding the milliseconds
4. countdown view:
...
- a primary button that initially says 'Start' and, when pressed starts the countdown and changes to 'Pause', which, when pressed, stops the countdown and changes back to 'Start', allowing or retake the countdown
- a secondary button called 'Reset' which, when pressed, stops the countdown and returns the countdown to 00:00:00
- when the countdown reaches 0 it must stop and do not go into negative values
...
- control that only valid time formats can be introduced when setting the objective countdown value
